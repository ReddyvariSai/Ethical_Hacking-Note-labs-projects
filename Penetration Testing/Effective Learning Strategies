...
A great deal of research has gone into how memory works and how we create strong memories and learn new skills. A full review of all the details is out of scope of this Module, but in short, we might summarize by saying that we can improve memory by doing the following:
...
Improve the way or mode in which we receive information: This could include multiple approaches. Information might be more easily retained if presented in multiple formats, such as videos or images. This might also include a distraction-free environment for the learner.
...
Labs
Most education research is focused on which group of individuals? Enter your answer in the form of a single character.
...
Verify
To help with retention, training materials may be presented in, for example, both text and video format. What is the name of this method of learning?
Answer

Verify
Ebbinghaus noted that after 20 minutes we forget roughly how much of what we learned. Enter your answer in the form of a single character.
...
Verify
The term "Cognitive Load" could be used to describe which of the following? Enter your answer in the form of a single character.
...
Some studies have been done on the difference between learning on a screen or from a book, and researchers have even explored whether the size of the screen matters. Interestingly, the research shows relevant information.

Among the findings are that smaller screens may make learning more difficult and that individuals who read books tend to understand the information more fully. There are payoffs and drawbacks to both approaches. Sometimes screen reading can cause visual or sensory fatigue. Learners learning in a digital context have easy access to several tools, including the ability to quickly and easily reference additional materials (for example, looking up the definition of a new vocabulary word). On the other hand, sometimes the act of reading forces one into a distraction-free environment that allows for deeper focus.

The second, and perhaps the more important thing to note here, has to do with a concept known as Contextual Learning. Although we can't explore all its details in this Module, this concept suggests that even on an intuitive level, we know that it's easier to learn how to build a house on a construction site.
...
The advantages that come from the peer support, community, and camaraderie of other learners in a traditional classroom setting are no longer a guarantee.
The pace and timing of the course is largely the learner's responsibility.
We will discuss some practical solutions to the second item shortly, but to connect with a wider community of learners, Offensive Security learners have a community of co-learners available on the OffSec Discord Server. There may also be local meetups or other communities available to a learner. Seeking out support and help (as well as helping and supporting others in turn) has benefits far beyond the classroom as well.

Let's pause at this point for a quick review of the material covered in this Learning Unit.

Labs
This section included high-level overviews of a variety of different studies. What conclusion did these studies come to? Enter your answer in the form of a single character.
...
Verify
Which of the following statements about preparing for an evolving information security landscape is incorrect? Enter your answer in the form of a single character.
...
Verify
Which of the following statements about the challenge of remote, asynchronous learning is incorrect? Enter your answer in the form of a single character.
...
The Challenge Labs
Proving Grounds
The training materials themselves will always trend toward focusing on scenarios that we can follow along with. There are times when we need to discuss a bit of theory so that we have enough background to go deeper, but in general, if the material can demonstrate working through a problem, then the expectation is that the learner should be able to follow along. Often a virtual machine (VM) is specifically built to accommodate this.
...
Let's consider the following experiment that was performed in 1978. A group of 8-year-old children were divided into two groups to practice a simple task: toss a small bean bag into a target hole. After being introduced to the task with the target at three feet (about 90 cm), the groups spent the next three months practicing. One group kept practicing with the target at the same distance. The other aimed at a pair of targets - practicing with distances of both two feet (60 cm) and four feet (120 cm).
...
Labs
What is meant by the term "Demonstration Method"? Enter your answer in the form of a single character.
...
Verify
Which of the following activities with OffSec training materials does not allow learners to apply their skills in a "hands-on" manner by themselves? Enter your answer in the form of a single character.
...
It may be difficult to understand some of these ideas about teaching and learning completely out of context. To observe some of these ideas "in action", let's take a moment to learn about something called executable permissions. We'll use this as a sort of case study to better understand how the OffSec training materials are presented and how we might approach learning.
...
We'll consider the fact that the chmod command itself is just a file. It follows the same rules as other files on the system, including the same rules about permissions. It exists in a slightly different location (in the /usr/bin/ directory) than our script, but the only reason we can run the chmod +x find_employee_names.py command at all is because the chmod file has its permissions set to allow us to run it as a program.
...
Labs
Which of the following is not a strategy used in teaching this section?
...
Verify
In this short Learning Unit, we focused on our central Module, the executable permission, but we also introduced and discussed other commands that allowed us to copy files and list file properties. Including some additional items like this is a strategy known as what?
A. Dual Coding 
B. Interleaving
...
Verify
At one point in this section, we tried running a command (chmod) on a file and it didn't work. Rather than quickly abandoning this line of thinking, we tried running the exact same command on itself, which also didn't work. This type of optimistic persistence is important to learn. OffSec has a two word name for this mentality. What is it?
Answer

Verify
Assume that in a normal Module, this material would have been accompanied by a virtual machine, which would have allowed the learner to follow along. The approach used here addresses several of the challenges discussed earlier in this Module. While this example wasn't perfect, which challenge is definitely not addressed in this particular teaching method (but can be addressed with the learner's approach to learning)?
...
4.5.1. Cornell Notes
There are many different note taking systems. Let's briefly examine one called Cornell Notes, which was developed by a Cornell University Professor named Walter Pauk in the 1950s. This method involves using a pen and paper, which helps with dual coding.
...
Retrieval Practice is, as the name suggests, the practice of determining whether information can be recalled. We can think of this simply as quizzing yourself.
...
The SQ3R method has learners follow a pattern of study activities - survey, question, read, recite, review. We will detail the SQ3R method here, but it is notably very similar to the PQ4R method, which is useful for reading comprehension. Learners who find the following tactic useful may want to check out the PQ4R method as well.
...
In the act of explaining things to children, we change our language to make things simpler. For example, when discussing a Brute Force Attack with another professional, we might quickly devolve into a discussion on the massive computational power needed to crack a certain key. While explaining it to a child, we could simply say "it's a way to keep guessing lots and lots of passwords until, hopefully, one of them works."
...
Addressing Bar Exam Stress: Practical Tips for Success
...
In addition to this resource, there are webinars, searchable blog posts, and YouTube videos of former learners reviewing their exam experiences. Heading into the exam with a clear understanding about what exactly it entails will not only reduce stress, but also improve performance.
